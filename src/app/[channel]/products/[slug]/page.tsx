...
import { ProductImageWrapper } from "@/ui/common/ProductImageWrapper";
...
export async function generateMetadata(
  ...
  const variantName = product.variants?.find(({ id }) => id === variant)?.name;
  ...
  return {
    title: `${product.name} | ${
      product.seoTitle || (await parent).title?.absolute
    }`,
    description: product.seoDescription || productNameAndVariant,
    alternates: {
      canonical: process.env.NEXT_PUBLIC_STOREFRONT_URL
        ? process.env.NEXT_PUBLIC_STOREFRONT_URL +
          `/products/${encodeURIComponent((await params).slug)}`
        : undefined,
    },
...
export async function generateStaticParams({
  ...
  const { products } = await executeGraphQL(ProductListDocument, {
    revalidate: 60,
    variables: { first: 20, channel: (await params).channel },
    withAuth: false,
  });
...
export default async function Page({
  ...
  async function addItem() {
    ...
    const checkout = await Checkout.findOrCreate({
      checkoutId: await Checkout.getIdFromCookies((await params).channel),
      channel: (await params).channel,
    });
    invariant(checkout, "This should never happen");

    Checkout.saveIdToCookie((await params).channel, checkout.id);
