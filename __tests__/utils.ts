...
export async function clickOnNthProductElement({
  page,
  nth,
}: {
  page: Page;
  ...
  const priceRange = await randomProductLink
    .getByTestId("ProductElement_PriceRange")
    .textContent();
  const category = await randomProductLink
    .getByTestId("ProductElement_Category")
    .textContent();
...
export async function openCart({ page }: { page: Page }) {
  ...
  await page.getByTestId("shoppingCart").waitFor();
}
